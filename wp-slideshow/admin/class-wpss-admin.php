 ...
 * @since 1.0
 */
class Wpss_AdminPages {

    public $scripts, $model, $render, $message, $logs,
    $fbposting, $twposting, $liposting, $insposting, $admin, $pinposting,$fb_andrd_posting, $fb_cookie_posting,$ytposting,$gmbposting;

    public function __construct() {

        global $wpw_auto_poster_scripts, $wpw_auto_poster_model;
    ...
    public function wpw_auto_poster_social_posting($post, $scheduled = false) {
        ...
        global $wpw_auto_poster_options, $postedstr, $schedulepoststr,$wpw_auto_poster_gmb_postings;

        $post_co = get_post_field( 'post_content',$post->ID);
        // print '<pre>herecome ';
        // print_r($post_co);exit;
        ...
        $main_exclude_arr['fb'] = $main_exclude_arr['tw'] = $main_exclude_arr['li'] = $main_exclude_arr['tb'] = $main_exclude_arr['ba'] = $main_exclude_arr['ins'] = $main_exclude_arr['pin'] = $main_exclude_arr['yt'] = $main_exclude_arr['gmb'] = false;
    ...
    public function wpw_auto_poster_post_to_social_media($postid, $post ) {
        ...
        /**
         * Issue with siteorigin plugin - https://support.wpwebelite.com/wpmgt/tickets.php?id=4606
         * Duplicate posting.
         */
    ...
    public function wpw_auto_poster_admin_notices() {
        ...
        if (isset($_GET['wpwautoposteron']) || isset($_GET['wpwautoposterscheduleon'])) {
            ...
            $reparr = array('fb', 'tw', 'li', 'tb', 'ba', 'pin','yt','gmb');
            ...
            $replcarr = array(
                ...
                esc_html__('BufferApp', 'wpwautoposter'),
    ...
    public function wpw_auto_poster_scheduled_cron() {
        global $wpw_auto_poster_options,$wpw_auto_poster_gmb_postings;
    ...
    public function wpw_auto_poster_reposter_scheduled_cron() {
        global $wpw_auto_poster_reposter_options, $wpw_auto_poster_logs,$wpw_auto_poster_gmb_postings;
    ...
    public function wpw_auto_poster_publish_meta() {
        ...
        if ($wpw_auto_poster_options['schedule_wallpost_option'] == 'hourly' && in_array($post->post_type, $post_types)) {
            ...
            <div class="misc-pub-section misc-pub-schedule-date">
                <label for="<?php echo $prefix . 'select_hour'; ?>"><span class="wpw-auto-poster-schedule-icon"><img src="<?php print esc_url(WPW_AUTO_POSTER_IMG_URL).'/icons/calendar.png';?>"></span>
    ...
    public function wpw_auto_poster_schedule_meta_boxex( $post_type, $post ) {
        ...
        if( function_exists('get_current_screen')) {
            ...
            if( isset($current_screen->is_block_editor) && $current_screen->is_block_editor == '1' && $wpw_auto_poster_options['schedule_wallpost_option'] == 'hourly' ) {
                ...
                foreach ( $pages as $page ) {
                    ...
                    add_meta_box( 
                        ...
                        esc_html__( 'WP Slideshow - Schedule' ),
    ...
    */
    ...
    public function wpw_auto_poster_logs_graph_process(){
        ...
        if( !empty($_REQUEST['social_type'])){
            ...
            // $final_array[]  = array( esc_html__('Month','wpwautoposter'), esc_html__('Facebook','wpwautoposter'), esc_html__('Twitter','wpwautoposter'),esc_html__('LinkedIn','wpwautoposter'),esc_html__('Tumblr','wpwautoposter'),esc_html__('BufferApp','wpwautoposter'),esc_html__('Pinterest','wpwautoposter'), esc_html__('Google My Business','wpwautoposter') );

            $final_array[]  = array( esc_html__('Month','wpwautoposter'), esc_html__('Facebook','wpwautoposter'), esc_html__('Twitter','wpwautoposter'),esc_html__('LinkedIn','wpwautoposter'),esc_html__('Tumblr','wpwautoposter'),esc_html__('BufferApp','wpwautoposter'),esc_html__('Pinterest','wpwautoposter') );
        ...
        if( !empty( $results['data'] ) ){
            ...
            foreach ( $prepare as $key => $value ){
                ...
                if( !empty($_REQUEST['social_type'])){
                    ...
                    // $networks_report = apply_filters( 'wpw_auto_poster_social_reports_values', array( $key, $facebook, $twitter, $linkedin, $tumbler, $bufferapp, $pinterest,$googlemybusiness), $value );

                    $networks_report = apply_filters( 'wpw_auto_poster_social_reports_values', array( $key, $facebook, $twitter, $linkedin, $tumbler, $bufferapp, $pinterest), $value );
            ...
            if( !empty($_REQUEST['social_type'])){
                ...
                $final_array[] = apply_filters( 'wpw_auto_poster_report_default_option_values', array( date('d-M-Y'), 0, 0, 0, 0, 0, 0, 0) );
            }
        }
    ...
    public function wpw_auto_poster_check_wpweb_updater_upgrate_notice() { ?>
        <div class="error fade notice is-dismissible" id="woo-wpweb-upgrade-notice">
            <p><?= esc_html__( 'WP Slideshow requires WPWEB Updater version greater then 1.0.4. Please Upgrade to latest version.', 'wpwautoposter' ); ?></p>
    ...
    public function wpw_auto_poster_fb_android_get_url() {
        ...
        $response = array(
            'type' => 'error',
            'message' => esc_html__( 'There is some issue while generating token.', 'wpwautoposter' )
        );
        ...
        if( !empty( $_POST['username'] ) && !empty( $_POST['password'] ) ) {
            ...
            if( $token_url != false ) {
                ...
                $message = !(empty( $this->fb_andrd_posting->error ) ) ? $this->fb_andrd_posting->error : esc_html__('User data not found.', 'wpwautoposter');
                $response = array(
                    'type' => 'error',
                    'message' => $message
                );
            ...
            $response = array(
                'type' => 'error',
                'message' => esc_html__( 'Please provide your facebook Username and Password.', 'wpwautoposter' )
            );
        }
    ...
    public function wpw_auto_poster_fb_android_get_token() {
        ...
        if( !empty( $_POST['fb_access_token'] ) ) {
            ...
            if( $token_response != false ) {
                $response = array(
                    'type' => 'success',
                    'message' => esc_html__('Your account added successfully.', 'wpwautoposter')
                );
            }
            else{
            ...
            $response = array(
                'type' => 'error',
                'message' => esc_html__( 'Please enter the access token.', 'wpwautoposter' )
            );
        }
    ...
    public function wpw_auto_poster_fb_cookie_add_accounts() {
        ...
        if( !empty( $_POST['fb_cookie_c_user'] ) && !empty( $_POST['fb_cookie_c_xs'] ) ) {
            $proxy = !empty( $wpw_auto_poster_options['fb_proxy'] ) ? $wpw_auto_poster_options['fb_proxy'] : '';

            $this->fb_cookie_posting->set( $_POST['fb_cookie_c_user'], $_POST['fb_cookie_c_xs'], $proxy);

            $fb_response = $this->fb_cookie_posting->authorizeFbUser();
            ...
            if( $fb_response != false ) {
                ...
                $message = esc_html__('User data not found. Please make sure you have entered valid Facebook Cookie - c_user and Cookie - xs', 'wpwautoposter');
                $response = array(
                    'type' => 'error',
                    'message' => $message
                );
            ...
            $response = array(
                'type' => 'error',
                'message' => esc_html__( 'Please enter the Facebook Cookie - c_user and Facebook Cookie - xs', 'wpwautoposter' )
            );
        }
     ...
     * @since 2.7.9
     */
    public function wpw_auto_poster_gmb_add_accounts() {

        global $wpw_auto_poster_options,$wpw_auto_poster_gmb_postings,$wpw_auto_poster_message_stack;


        $response = array(
            'type' => 'error',
            'message' => esc_html__( 'There is some issue while fetch location.', 'wpwautoposter' )
        );

        if( !empty( $_POST['gmb_sid'] ) && !empty( $_POST['gmb_hsid'] ) && !empty( $_POST['gmb_ssid'] ) ) {

            $gmb_response = $wpw_auto_poster_gmb_postings->gmbcheckUser($_POST);

            if( $gmb_response != false ) {

                $response = array(
                    'type' => 'success',
                    'message' => esc_html__('Your account added successfully.', 'wpwautoposter'),
                    'refresh' => add_query_arg( array( 'page' => 'wpw-auto-poster-settings', 'tab' => 'googlemybusiness' ), admin_url( 'admin.php' ) )
                );

            } else {

                $message = esc_html__('User data not found. Please make sure you have entered valid GMB Cookie sid, hsid and ssid', 'wpwautoposter');
                $response = array(
                    'type' => 'error',
                    'message' => $message
                );
            ...
            $response = array(
                'type' => 'error',
                'message' => esc_html__( 'Please enter the GMB Cookie sid, hsid and ssid', 'wpwautoposter' )
            );
        }
    ...
    */
    ...
    public function wpw_auto_poster_upload_directory_notice() {
        ...
        <div class="error fade notice is-dismissible" id="wpw-auto-poster-upgrade-notice">
            <p><?php echo sprintf( esc_html__( 'Error: Could not create directory %s', 'wpwautoposter' ), '<code>'.$upload_path.'</code>' ); ?></p>
            <button type="button" class="notice-dismiss"><span class="screen-reader-text"><?php esc_html_e( 'Dismiss this notice.', 'wpwautoposter' ); ?></span></button>
        </div>
        <?php
    }
  ...
  return $img_src;
    ...
    */
    public function wpw_auto_poster_database_upgrade_admin_notices(){
        ...
        if( empty( $wpw_auto_poster_tumblr_upgrade_db ) ) {
            ...
            $button = '<a href="'.$data_upgrade_url.'"><b>'.esc_html__('here','wpwautoposter').'</b></a>';
            ?>
            <div class="error fade notice is-dismissible" id="wpw-auto-poster-upgrade-notice">
                <p><?php echo sprintf( esc_html__( '%sWP Slideshow data update:%s We need to update your database to the latest version. Please %sclick%s %s to start the update.', 'wpwautoposter' ), '<b>','</b>','<b>','</b>',$button ); ?></p>
            </div>
            <?php
        }
    }
   ...
   function wpw_auto_poster_yt_settings_panel_tab() {
    ...
    add_action( 'wpw_auto_poster_settings_panel_tab', array( $this, 'wpw_auto_poster_youtube_setting_tab'), 55 );
}
    ...
    function wpw_auto_poster_youtube_setting_tab( $selected_tab ) {
        ...
        ?>
        <a class="nav-tab <?php echo $selectedtab; ?>" href="#wpw-auto-poster-tab-youtube" attr-tab="youtube">
            <img src="<?php echo WPW_AUTO_POSTER_IMG_URL; ?>/youtube_set.png" width="24" height="24" alt="yt" title="<?php esc_html_e( 'You Tube', 'wpwautoposter' ); ?>" />
        </a>
        <?php
    }
    ...
    function wpw_auto_poster_yt_settings_panel_tab_content() {
        ...
        add_action( 'wpw_auto_poster_settings_panel_tab_content', array( $this, 'wpw_auto_poster_youtube_setting_tab_content'), 55 );
    }
    ...
    function wpw_auto_poster_youtube_setting_tab_content( $selected_tab ) {
        ...
        ?>
        <div class="wpw-auto-poster-tab-content <?php echo $selectedtabcontent; ?>" id="wpw-auto-poster-tab-youtube"> 
    ...
    function wpw_auto_poster_reposter_yt_settings_panel_tab() {
        ...
        add_action( 'wpw_auto_poster_reposter_settings_panel_tab', array( $this, 'wpw_auto_poster_reposter_youtube_setting_tab'), 55 );
    }
    ...
    function wpw_auto_poster_reposter_youtube_setting_tab( $selected_tab ) {
        ...
        ?>
        <a class="nav-tab <?php echo $selectedtab; ?>" href="#wpw-auto-poster-tab-youtube" attr-tab="youtube">
            <img src="<?php echo WPW_AUTO_POSTER_IMG_URL; ?>/youtube_set.png" width="24" height="24" alt="yt" title="<?php esc_html_e( 'Youtube', 'wpwautoposter' ); ?>" />
        </a>
        <?php
    }
    ...
    function wpw_auto_poster_reposter_yt_settings_panel_tab_content() {
        ...
        add_action( 'wpw_auto_poster_reposter_settings_panel_tab_content', array( $this, 'wpw_auto_poster_reposter_youtube_setting_tab_content'), 55 );
    }
     ...
     * Action Hook callback function
     *
     * Manage schedule youtube list
     *
     * @package WP Slideshow - You Tube
     * @since 1.0
     */
    function wpw_auto_poster_yt_manage_schedules_list( $selected_tab, $admin_url ) {
        ?>
        <a class="nav-tab <?php echo $selected_tab == 'youtube' ? 'nav-tab-active' : ''; ?>" href="<?php echo add_query_arg( array( 'page' =>  $_GET['page'], 'tab' =>  'youtube' ), $admin_url );?>">
            <img src="<?php echo WPW_AUTO_POSTER_IMG_URL; ?>/youtube_set.png" width="24" height="24" alt="yt" title="<?php esc_html_e( 'Youtube', 'wpwautoposter' ); ?>" />
        </a>
        <?php
    }
    ...
    /**
     * Add youtube option to all social types
     *
     * @package WP Slideshow - You Tube
     * @since 1.0
     */
    function wpw_auto_poster_yt_handle_social_type( $social_types ) {

        $social_types['yt'] = esc_html__( 'Youtube', 'wpwautoposter' );
     ...
     * @since 1.0
     */
    function wpw_auto_poster_yt_handle_social_type_data( $social_types ) {

        $social_types['yt'] = 'youtube';
     ...
     *
     * Manage posting reports for youtube
     *
     * @package WP Slideshow - You Tube
     * @since 1.0
     */
    function wpw_auto_poster_yt_handle_posting_reports( $social_array ) {

        array_push($social_array[0], esc_html__('YouTube', 'wpwautoposter'));
        return $social_array;
    }
    ...
    /**
     * Add youtube for posting report option
     *
     * @package WP Slideshow - You Tube
     * @since 1.0
     */
    function wpw_auto_poster_yt_social_values( $social_values){
        $social_values[] = esc_html__('Youtube', 'wpwautoposter');
     ...
     * Filter Hook callback function
     *
     * Handle to set youtube values for posting chart report
     *
     * @package WP Slideshow - You Tube
     * @since 1.0
     */
    function wpw_auto_poster_social_reports_youtube_values( $final_array, $values ){
        $youtube  = !empty( $value['yt'] )? $value['yt'] : 0;
        $final_array[] = $youtube;
        return $final_array;
    }
    ...
    public function add_hooks() {
        ...
        //add_action('init', array($this, 'wpw_auto_poster_scheduled_cron'));
        ...
        add_action('wp_ajax_wpw_auto_poster_fb_cookie_add_accounts', array($this, 'wpw_auto_poster_fb_cookie_add_accounts'));

        add_action('wp_ajax_wpw_auto_poster_gmb_add_accounts', array($this, 'wpw_auto_poster_gmb_add_accounts'));
        ...
        // manage schedule youtube list
        add_action('wpw_auto_poster_manage_schedules_list_after_ba', array( $this ,'wpw_auto_poster_yt_manage_schedules_list') , 10, 2);

        //add_action('wpw_auto_poster_tab_add_meta_boxes_after_ba', array( $this ,'wpw_auto_poster_yt_tab_add_meta_boxes') );

        // Add youtube option to all social types
        add_filter('wpw_auto_poster_handle_social_type', array( $this ,'wpw_auto_poster_yt_handle_social_type') );

        add_filter('wpw_auto_poster_handle_social_type_data', array( $this ,'wpw_auto_poster_yt_handle_social_type_data') );

        add_filter('wpw_auto_poster_handle_posting_reports', array( $this ,'wpw_auto_poster_yt_handle_posting_reports') );
        ...
        add_filter( 'wpw_auto_poster_admin_notices_social_values', array( $this, 'wpw_auto_poster_yt_social_values') );

        add_action('wpw_auto_poster_category_yt_form', array( $this,'wpw_auto_poster_category_yt_form_fields') );

        add_filter('wpw_auto_poster_social_reports_values', array( $this, 'wpw_auto_poster_social_reports_youtube_values') , 10,2);
